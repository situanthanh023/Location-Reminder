...
class RemindersActivityTest : AutoCloseKoinTest() {// Extended Koin Test - embed autoclose @after method to close Koin after every test
    ...
    @After
    fun cleanUp_UnregisterIdlingResource() {
        IdlingRegistry.getInstance().unregister(dataBindingIdlingResource)
    }

    @Test
    fun test_WithErrorEnterTitleSnackBar() {
    ...
    @Test
    fun test_WithReminderSavedToastMessage() {
        val activity = ActivityScenario.launch(RemindersActivity::class.java)
        dataBindingIdlingResource.monitorActivity(activity)
        ...
        onView(withText(R.string.reminder_saved)).inRoot(withDecorView(not(`is`(getActivityContext(activity).window.decorView))))
